...
void bookSeat(Bus &bus)
{
    ...
    if (!bus.seats[seatNum - 1])
    {
        cout << "Seat " << seatNum << " is already booked.\n";
        return;
    }

    bus.seats[seatNum - 1] = false;
    bus.availableSeats--;
...
void addStandbyPassenger(Bus &bus)
{
    ...
    int standbyPassno = bus.numStandbyPassengers + 1;
    bus.standbyPassengers[bus.numStandbyPassengers] = standbyPassno;
    bus.numStandbyPassengers++;

    cout << "Added to the standby list. Your standby number is: " << standbyPassno << "\n";
}
...
int main()
{
    ...
    int choice = 0;
mainmenu:
    cout << "*************************** WELCOME TO DAEWOO BUS SERVICE *****************************" << endl;
    ...
    if (choice == 1)
    {
        ...
        if (searchResult == 0)
        {
            cout << "No buses found for the given search criteria." << endl;
        }
        else
        {
            int busIndex;
